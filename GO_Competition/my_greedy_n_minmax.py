...
class MyFinalPlayer():
    ...
    def return_first_result(self, go, piece_type):
    ...
    def return_second_result(self, go, piece_type):
    ...
    def get_input(self, go, piece_type):
        ...
        return self.get_my_input(go, piece_type, valid_moves)

    def get_my_input(self, go, piece_type, valid_moves):
        for move in valid_moves:
            state = self.get_current_game_state(go)
            state.place_chess(move[0], move[1], piece_type)
            board_hash = self.encode_state(state.board, state.size)  # type: int
            if board_hash in self.move_q_values:
                return self.return_first_result(go, piece_type)
        return self.return_second_result(go, piece_type)